...
extension CPU.Cache {
    // SAFETY: The type encapsulates the unsafe pointer behind a checked API
    // SAFETY: (init consumes T into the slot, deinit deinitializes and
    // SAFETY: deallocates, `value` is the only accessor and runs under
    // SAFETY: coroutine borrow/exclusive semantics). Consumers don't need
    // SAFETY: to annotate use with `unsafe`. Encapsulation invariant per
    // SAFETY: [MEM-SAFE-021].
    ...
    public struct Padded<T: ~Copyable>: ~Copyable {
        ...
        /// The 128-byte-aligned heap storage.
        ///
        /// Non-null for the lifetime of `self`; deallocated in `deinit`.
        @usableFromInline
